...
function handleHistoryNavigation() {
  ...
  if (scanMatch) {
    ...
    fetch(`/api/scan/${encodeURIComponent(scanId)}/status`, { cache: "no-store" })
      ...
      .then((st) => {
        if (!st || !st.status || !["queued", "running", "done", "saved"].includes(st.status)) {
          showActionBanner("Scan not found / expired", [
            { label: "Go Home", onClick: () => goHome(true, false) },
          ]);
          return;
  ...
  if (eventType === "saved") {
    markScanSettled(eventScanId, {
      downloads: {
        ...
        full_static: `/api/download/${encodeURIComponent(eventScanId)}/html-full`,
...
function probeResultFragment(scanId, retries) {
  ...
  let attempt = 0;
  const run = () => fetch(`/api/result-fragment/${encodeURIComponent(scanId)}?_t=${Date.now()}`, { cache: "no-store" })
    .then((r) => r.ok)
    .catch(() => false)
    .then((ok) => {
      if (ok) { return true; }
...
function loadResults(scanId, opts) {
  ...
  if (cacheEntry && (Date.now() - Number(cacheEntry.ts || 0) < 300000)) {
    return Promise.resolve(cacheEntry.html).then((html) => {
      ...
      AppStore.currentReport = null;
      AppStore.currentReportScanId = scanId;
    });
  }
  fetch(`/api/result-fragment/${encodeURIComponent(scanId)}`)
...
function renderSubdomainBrowser(scanId) {
  ...
  const limit = Math.max(1, Number(state.limit || 50));
  const page = Math.max(1, Number(state.page || 1));
  const filteredTotal = Math.max(0, Number(state.filteredTotal || state.total || rows.length || 0));
  const total = Math.max(0, Number(state.total || filteredTotal || rows.length || 0));
  const start = filteredTotal ? (((page - 1) * limit) + 1) : 0;
  ...
  `).join("") || `<tr><td colspan="9" class="mono">${escapeHtml(state.error || (state.loading ? "Loading subdomains..." : (state.loaded ? "No subdomains matched the current filter." : "Subdomains load on demand.")))}</td></tr>`;
  if (meta) {
    if (state.loading) {
      meta.textContent = "Loading subdomains...";
    } else if (state.error) {
      meta.textContent = state.error;
    } else if (state.loaded) {
      meta.textContent = `${start ? `Showing ${start}-${end}` : "Showing 0"} of ${filteredTotal.toLocaleString()}${state.query ? " matches" : " subdomains"}${state.query ? ` · ${total.toLocaleString()} total` : ""}`;
    } else {
      meta.textContent = "Subdomains load on demand.";
    }
  }
  const totalPages = Math.max(1, Math.ceil(filteredTotal / limit));
  if (prev) { prev.disabled = state.loading || page <= 1; }
  if (next) { next.disabled = state.loading || filteredTotal === 0 || page >= totalPages; }
}
...
function renderArchiveBrowser(scanId) {
  ...
  const limit = Math.max(1, Number(state.limit || 50));
  const filteredTotal = Math.max(0, Number(state.filteredTotal || 0));
  const total = Math.max(0, Number(state.total || 0));
  const start = filteredTotal ? (((Math.max(1, Number(state.page || 1)) - 1) * limit) + 1) : 0;
  ...
  `).join("") || `<tr><td colspan="4" class="mono">${escapeHtml(state.error || (state.loading ? "Loading archive URLs..." : (state.loaded ? "No archive URLs matched the current filter." : "Archive URLs load on demand.")))}</td></tr>`;
  if (meta) {
    if (state.loading) {
      meta.textContent = "Loading archive URLs...";
    } else if (state.error) {
      meta.textContent = state.error;
    } else if (state.loaded) {
      meta.textContent = `${start ? `Showing ${start}-${end}` : "Showing 0"} of ${filteredTotal.toLocaleString()}${state.query ? " matches" : " archive URLs"}${state.query ? ` · ${total.toLocaleString()} total` : ""}`;
    } else {
      meta.textContent = "Loading archive URLs...";
    }
  }
  ...
  if (next) { next.disabled = state.loading || filteredTotal === 0 || state.page >= totalPages; }
}

function loadSubdomainBrowser(scanId) {
  const root = $("subdomain-browser");
  const tbody = $("sub-table-body");
  if (!scanId || !root || !tbody) { return Promise.resolve(); }
  root.dataset.loadedOnce = "1";
  const state = largeBrowserState(scanId, "subdomains");
  ...
  return fetch(`/api/result-data/${encodeURIComponent(scanId)}/subdomains?${params.toString()}`, { cache: "no-store" })
    .then((r) => (r.ok ? r.json() : Promise.reject(new Error(`HTTP ${r.status}`))))
    .then((payload) => {
      if (requestId !== state.requestId) { return; }
      state.items = Array.isArray(payload.data) ? payload.data.slice() : [];
      state.total = Number(payload.total || state.items.length || 0);
      state.filteredTotal = Number(payload.filtered_total || state.total || state.items.length || 0);
      state.limit = Math.max(1, Number(payload.limit || limit));
    ...
    .catch((err) => {
      if (requestId !== state.requestId) { return; }
      state.error = `Failed to load subdomains: ${err.message || "unknown error"}`;
      state.loaded = false;
      state.items = [];
      renderSubdomainBrowser(scanId);
    })
...
function loadArchiveBrowser(scanId) {
  ...
  return fetch(`/api/result-data/${encodeURIComponent(scanId)}/archive?${params.toString()}`, { cache: "no-store" })
    .then((r) => (r.ok ? r.json() : Promise.reject(new Error(`HTTP ${r.status}`))))
    .then((payload) => {
      if (requestId !== state.requestId) { return; }
      state.items = Array.isArray(payload.items) ? payload.items.slice() : [];
      state.total = Number(payload.total || 0);
      state.filteredTotal = Number(payload.filtered_total || state.items.length || 0);
      state.limit = Math.max(1, Number(payload.limit || limit));
      state.hasMore = Boolean(payload.has_more);
      state.loaded = true;
      state.error = "";
    ...
    .catch((err) => {
      if (requestId !== state.requestId) { return; }
      state.error = `Failed to load archive browser: ${err.message || "unknown error"}`;
      state.loaded = false;
      state.items = [];
      renderArchiveBrowser(scanId);
    })
...
function observeLargeBrowser(root, onVisible) {
  if (!root || typeof onVisible !== "function") { return; }
  if (root.dataset.observeBound === "1") { return; }
  root.dataset.observeBound = "1";
  const trigger = () => {
    if (root.dataset.loadedOnce === "1") { return; }
    root.dataset.loadedOnce = "1";
    onVisible();
  };
...
function initLargeResultBrowsers(scanId) {
  if (!scanId) { return; }
  const subRoot = $("subdomain-browser");
  ...
  if (subSearch && subSearch.dataset.bound !== "1") {
    ...
    subSearch.addEventListener("input", () => {
      const state = largeBrowserState(scanId, "subdomains");
      state.query = String(subSearch.value || "").trim();
      state.page = 1;
      clearTimeout(state.searchTimer);
      state.searchTimer = setTimeout(() => loadSubdomainBrowser(scanId), 250);
    });
  }
  ...
  if (subNext && subNext.dataset.bound !== "1") {
    ...
    subNext.addEventListener("click", () => {
      const state = largeBrowserState(scanId, "subdomains");
      const totalPages = Math.max(1, Math.ceil(Math.max(0, Number(state.filteredTotal || 0)) / Math.max(1, Number(state.limit || 50))));
      state.page = Math.min(totalPages, state.page + 1);
      loadSubdomainBrowser(scanId);
    });
  }
  const subState = largeBrowserState(scanId, "subdomains");
  subState.limit = Math.max(1, Number(subState.limit || 50));
  ...
  if (archiveSearch && archiveSearch.dataset.bound !== "1") {
    ...
    archiveSearch.addEventListener("input", () => {
      const state = largeBrowserState(scanId, "archive");
      state.query = String(archiveSearch.value || "").trim();
      state.page = 1;
      clearTimeout(state.searchTimer);
      state.searchTimer = setTimeout(() => loadArchiveBrowser(scanId), 250);
    });
  }
  ...
  renderSubdomainBrowser(scanId);
  renderArchiveBrowser(scanId);
  observeLargeBrowser(subRoot, () => loadSubdomainBrowser(scanId));
  observeLargeBrowser($("archive-browser"), () => loadArchiveBrowser(scanId));
}
...
function currentResultsScanId() {
  return (($("results-page") && $("results-page").dataset.scanId) || AppStore.currentScanId || AppStore.viewedScanId || "");
}

function standaloneLimitNote(state) {
  const message = String((state && state.payload && state.payload.standalone_limit_reason) || "").trim();
  return message ? `<div class="lazy-preview-note">${escapeHtml(message)}</div>` : "";
}

function sectionSupportsServerSearch(section) {
  return ["subdomains", "emails", "ips", "certs", "techs", "vulns", "dorks", "cloud", "archive"].includes(String(section || ""));
}

function emailPatternMeta() {
  const shell = findSectionShell("emails");
  return {
    pattern: String(shell?.dataset.emailPattern || "").trim(),
    confidence: String(shell?.dataset.emailPatternConfidence || "").trim(),
  };
}
...
function renderSubdomainsSection(section, rows, state) {
  ...
  const filteredTotal = Number(state.filteredTotal || totalAvailable || rows.length || 0);
  if (!rows.length && filteredTotal === 0) {
    return renderEmptyState(q ? "No subdomains matched the current filter." : "No subdomains captured.");
  }
  const body = rows.map((row) => `
    <tr>
      <td class="mono copyable-cell" data-copy-value="${escapeHtml(row.name || row.subdomain || row.host || "")}">${escapeHtml(row.name || row.subdomain || row.host || "")}</td>
      <td>${hostnameTagList(row.name || row.subdomain || row.host || "")} ${(row.tags || []).slice(0, 6).map((tag) => `<span class="badge badge-gray">${escapeHtml(tag)}</span>`).join(" ")}</td>
  ...
  `).join("");
  const note = `
    <div class="lazy-preview-note"><strong>${totalAvailable.toLocaleString()} subdomains found</strong>${q ? ` · ${filteredTotal.toLocaleString()} match filter` : ""} · showing ${rows.length.toLocaleString()}${filteredTotal > rows.length ? ` of ${filteredTotal.toLocaleString()}` : ""}</div>
    ${state.hasMore ? '<div class="lazy-preview-note">Load more to continue browsing subdomains without rendering the full inventory at once.</div>' : ""}
    ${standaloneLimitNote(state)}
  `;
...
function renderEmailsSection(section, rows, state) {
  ...
  const filteredTotal = Number(state.filteredTotal || totalAvailable || rows.length || 0);
  if (!rows.length && filteredTotal === 0) {
    return renderEmptyState(q ? "No emails matched the current filter." : "No emails captured.");
  }
  const body = rows.map((row) => `
    <tr>
  ...
  `).join("");
  const emailPattern = emailPatternMeta();
  return renderTableShell("email-table", [
    { label: "Email" }, { label: "Role" }, { label: "Sources" }, { label: "Evidence Source" }, { label: "Confidence" },
  ], body, `<div class="lazy-preview-note"><strong>${totalAvailable.toLocaleString()} emails found</strong>${q ? ` · ${filteredTotal.toLocaleString()} match filter` : ""} · showing ${rows.length.toLocaleString()}${filteredTotal > rows.length ? ` of ${filteredTotal.toLocaleString()}` : ""}</div>${emailPattern.pattern ? `<div class="lazy-preview-note">Pattern detected: ${escapeHtml(emailPattern.pattern)}${emailPattern.confidence ? ` (${escapeHtml(String(emailPattern.confidence))}% confidence)` : ""}</div>` : ""}${state.hasMore ? '<div class="lazy-preview-note">Load more to continue browsing emails without rendering the full inventory at once.</div>' : ""}${standaloneLimitNote(state)}`);
}

function renderIpsSection(section, rows, state) {
  const q = String(state.query || "").trim();
  const filteredTotal = Number(state.filteredTotal || state.total || rows.length || 0);
  if (!rows.length && filteredTotal === 0) {
    return renderEmptyState(q ? "No infrastructure records matched the current filter." : "No infrastructure records captured.");
  }
  const selectionKey = currentResultsScanId();
  const quickFilter = AppStore.infraFilters[selectionKey] || "all";
  const scoped = rows.filter((row) => infraFilterMatch(row, quickFilter));
  if (!scoped.length) {
    return renderEmptyState("No infrastructure records matched the current quick filter in the loaded slice.");
  }
  const selectedIp = AppStore.infraSelections[selectionKey] || (scoped[0] && scoped[0].ip) || "";
  const selectedRow = scoped.find((row) => String(row.ip || "") === selectedIp) || scoped[0] || rows[0];
  if (selectedRow) {
  ...
  return `
    ...
    <div class="infra-console">
      <div class="infra-table-pane">
        ...
        ], body, `<div class="lazy-preview-note"><strong>${Number(state.total || rows.length || 0).toLocaleString()} infrastructure records</strong>${q ? ` · ${filteredTotal.toLocaleString()} match filter` : ""} · showing ${scoped.length.toLocaleString()}${filteredTotal > rows.length ? ` of ${filteredTotal.toLocaleString()}` : ""}</div>${state.hasMore ? '<div class="lazy-preview-note">Load more to continue browsing infrastructure records without rendering the full inventory at once.</div>' : ""}${standaloneLimitNote(state)}`)}
      </div>
...
function renderCertsSection(section, rows, state) {
  ...
  const filteredTotal = Number(state.filteredTotal || totalAvailable || rows.length || 0);
  if (!rows.length && filteredTotal === 0) {
    return renderEmptyState(q ? "No certificate records matched the current filter." : "No certificate records captured.");
  }
  const body = rows.map((row) => `
    <tr>
  ...
  ], body, `<div class="lazy-preview-note"><strong>${totalAvailable.toLocaleString()} certificates</strong>${q ? ` · ${filteredTotal.toLocaleString()} match filter` : ""} · showing ${rows.length.toLocaleString()}${filteredTotal > rows.length ? ` of ${filteredTotal.toLocaleString()}` : ""}</div>${state.hasMore ? '<div class="lazy-preview-note">Load more to continue browsing certificate records without rendering the full inventory at once.</div>' : ""}${standaloneLimitNote(state)}`);
}

function renderTechnologiesSection(section, rows, state) {
  const q = String(state.query || "").trim();
  const totalAvailable = Number(state.total || rows.length || 0);
  const filteredTotal = Number(state.filteredTotal || totalAvailable || rows.length || 0);
  if (!rows.length && filteredTotal === 0) { return renderEmptyState(q ? "No technologies matched the current filter." : "No technologies captured."); }
  ...
  `).join("");
  return `<div class="lazy-preview-note"><strong>${totalAvailable.toLocaleString()} technologies</strong>${q ? ` · ${filteredTotal.toLocaleString()} match filter` : ""} · showing ${rows.length.toLocaleString()}${filteredTotal > rows.length ? ` of ${filteredTotal.toLocaleString()}` : ""}</div>${state.hasMore ? '<div class="lazy-preview-note">Load more to continue browsing technologies without rendering the full catalog at once.</div>' : ""}${standaloneLimitNote(state)}<div class="tech-grid">${cards}</div>`;
}
...
function renderVulnsSection(section, rows, state) {
  ...
  const filteredTotal = Number(state.filteredTotal || totalAvailable || rows.length || 0);
  if (!rows.length && filteredTotal === 0) {
    return renderEmptyState(q ? "No vulnerability findings matched the current filter." : "No vulnerability findings captured.");
  }
  ...
  ], body, `<div class="lazy-preview-note"><strong>${totalAvailable.toLocaleString()} vulnerabilities</strong>${q ? ` · ${filteredTotal.toLocaleString()} match filter` : ""} · CRITICAL ${sevCounts.CRITICAL} · HIGH ${sevCounts.HIGH} · MEDIUM ${sevCounts.MEDIUM} · LOW ${sevCounts.LOW}</div><div class="lazy-preview-note">Showing ${rows.length.toLocaleString()}${filteredTotal > rows.length ? ` of ${filteredTotal.toLocaleString()}` : ""}</div>${state.hasMore ? '<div class="lazy-preview-note">Load more to continue browsing vulnerabilities without rendering the full table at once.</div>' : ""}${standaloneLimitNote(state)}`);
}

function renderDorksSection(section, rows, state) {
  const q = String(state.query || "").trim();
  const totalAvailable = Number(state.total || rows.length || 0);
  const filteredTotal = Number(state.filteredTotal || totalAvailable || rows.length || 0);
  if (!rows.length && filteredTotal === 0) { return renderEmptyState(q ? "No dorks matched the current filter." : "No dorks captured."); }
  const body = rows.map((row) => `
    <tr>
      <td>${escapeHtml(row.source || "")}</td>
  ...
  ], body, `<div class="lazy-preview-note"><strong>${totalAvailable.toLocaleString()} dorks</strong>${q ? ` · ${filteredTotal.toLocaleString()} match filter` : ""} · showing ${rows.length.toLocaleString()}${filteredTotal > rows.length ? ` of ${filteredTotal.toLocaleString()}` : ""}</div>${state.hasMore ? '<div class="lazy-preview-note">Load more to continue browsing dorks without rendering the full table at once.</div>' : ""}${standaloneLimitNote(state)}`);
}

function renderCloudSection(section, rows, state) {
  const q = String(state.query || "").trim();
  const totalAvailable = Number(state.total || rows.length || 0);
  const filteredTotal = Number(state.filteredTotal || totalAvailable || rows.length || 0);
  if (!rows.length && filteredTotal === 0) { return renderEmptyState(q ? "No cloud assets matched the current filter." : "No cloud assets captured."); }
  const body = rows.map((row) => `
    <tr>
  ...
  ], body, `<div class="lazy-preview-note"><strong>${totalAvailable.toLocaleString()} cloud assets</strong>${q ? ` · ${filteredTotal.toLocaleString()} match filter` : ""} · showing ${rows.length.toLocaleString()}${filteredTotal > rows.length ? ` of ${filteredTotal.toLocaleString()}` : ""}</div>${state.hasMore ? '<div class="lazy-preview-note">Load more to continue browsing cloud assets without rendering the full table at once.</div>' : ""}${standaloneLimitNote(state)}`);
}

function renderArchiveSection(payload, state) {
  const rows = Array.isArray(state.items) && state.items.length
    ? state.items
    : (Array.isArray(payload.items) ? payload.items : normalizeRowsData(payload));
  const q = String(state.query || sectionSearchQuery("archive") || "").trim();
  const total = Number((payload && payload.total) || state.total || rows.length || 0);
  const filteredTotal = Number((payload && payload.filtered_total) || state.filteredTotal || rows.length || 0);
  if (!rows.length && filteredTotal === 0) {
    return renderEmptyState(q ? "No archive URLs matched the current filter." : "No archive URLs captured.");
  }
  const archiveBody = rows.map((row) => `
    <tr>
      <td class="mono" style="word-break:break-all"><a href="${escapeHtml(row.url || "")}" target="_blank">${escapeHtml(row.url || "")}</a></td>
      <td>${escapeHtml(row.timestamp || "")}</td>
      <td>${escapeHtml(String(row.status_code || ""))}</td>
      <td>${escapeHtml(row.mime_type || "")}</td>
    </tr>
  `).join("");
  return renderTableShell("archive-all-urls", [
    { label: "URL" }, { label: "Timestamp" }, { label: "Status" }, { label: "MIME" },
  ], archiveBody, `
    <div class="lazy-preview-note"><strong>${total.toLocaleString()} archive URLs</strong>${q ? ` · ${filteredTotal.toLocaleString()} match filter` : ""} · loaded ${rows.length.toLocaleString()}${filteredTotal > rows.length ? ` of ${filteredTotal.toLocaleString()}` : ""}</div>
    ${state.hasMore ? '<div class="lazy-preview-note">Load more to continue browsing archived URLs without loading the full corpus into the browser.</div>' : ""}
    ${standaloneLimitNote(state)}
  `);
}
...
function renderSectionState(scanId, section) {
  ...
  if (status) {
    const readyCount = Array.isArray(state.items) ? Number(state.items.length || 0) : Number(state.total || 0);
    const readyTotal = Number(state.filteredTotal || state.total || readyCount || 0);
    const readyLabel = readyTotal
      ? `${readyCount}/${readyTotal} record(s) ready.`
      : `${readyCount} record(s) ready.`;
    status.textContent = state.loading
      ? "Loading..."
      : (state.loaded ? readyLabel : "Loading on demand.");
  }
  if (!content) {
    if (section === "asn") { renderSectionState(scanId, "ips"); }
    return;
  ...
  if (!state.payload) {
    if (state.loading) {
      content.innerHTML = renderEmptyState("Loading section...");
    }
    return;
  }
  ...
  if (loadMoreNeeded) {
    ...
    btn.type = "button";
    btn.className = "btn btn-ghost btn-sm";
    const totalAvailable = Number(state.filteredTotal || state.total || state.items.length || 0);
    btn.textContent = `Load more (${state.items.length}/${totalAvailable})`;
...
function loadSectionData(scanId, section, opts) {
  ...
  const isAppend = Boolean(opts && opts.append);
  const query = sectionSupportsServerSearch(section) ? sectionSearchQuery(section) : "";
  const queryChanged = !isAppend && query !== String(state.query || "");
  if (state.loading && !queryChanged) { return Promise.resolve(); }
  if (queryChanged) {
    state.items = [];
    state.total = 0;
  ...
  if (state.loaded && !isAppend && !queryChanged) {
  ...
  state.query = query;
  renderSectionState(scanId, section);
  const nextOffset = isAppend ? state.offset : 0;
  const limit = ({
    archive: 50,
    subdomains: 50,
    emails: 100,
  ...
  state.limit = limit;
  return fetch(`${shell.dataset.endpoint}?${params.toString()}`, { cache: "no-store" })
    .then((r) => (r.ok ? r.json() : Promise.reject(new Error(`HTTP ${r.status}`))))
    .then((payload) => {
      if (requestSeq !== state.requestSeq) { return; }
      if (section === "archive") {
        const pageItems = Array.isArray(payload.items) ? payload.items : [];
        state.items = isAppend ? state.items.concat(pageItems) : pageItems.slice();
        state.offset = Number(payload.offset || nextOffset) + pageItems.length;
        state.total = Number(payload.total || 0);
        state.filteredTotal = Number(payload.filtered_total || state.items.length || 0);
        state.hasMore = Boolean(payload.has_more);
        state.payload = { ...payload, data: state.items.slice(), items: state.items.slice() };
      } else if (Array.isArray(payload.data)) {
        state.items = isAppend ? state.items.concat(payload.data) : payload.data.slice();
        state.offset = Number(payload.offset || nextOffset) + payload.data.length;
        state.total = Number(payload.total || state.items.length || 0);
        state.filteredTotal = Number(payload.filtered_total || payload.total || state.items.length || 0);
        state.hasMore = Boolean(payload.has_more);
        state.payload = { ...payload, data: state.items.slice() };
      } else {
        state.payload = payload;
        state.total = Number(payload.total || 0);
        state.filteredTotal = Number(payload.filtered_total || state.total || 0);
        state.hasMore = false;
      }
      state.loaded = true;
    })
...
function bindSectionSearch(scanId) {
  qsa("[data-section-search]").forEach((input) => {
    ...
    input.addEventListener("input", () => {
      ...
      if (sectionSupportsServerSearch(section)) {
        const state = sectionCacheEntry(scanId, section);
        clearTimeout(state.searchTimer);
        state.searchTimer = setTimeout(() => loadSectionData(scanId, section), 250);
        return;
      }
      renderSectionState(scanId, section);
    });
